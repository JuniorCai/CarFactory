...
namespace CarFactory.Tests
{
    public abstract class CarFactoryTestBase : AbpIntegratedTestBase<CarFactoryTestModule>
    {
        ...
        protected void UsingDbContext(Action<CarFactoryDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<CarFactoryDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<CarFactoryDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<CarFactoryDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<CarFactoryDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CarFactoryDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<CarFactoryDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CarFactoryDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<CarFactoryDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<CarFactoryDbContext, Task<T>> func)
        {
            T result;
