...
namespace NbscwMPACarFactory.Tests
{
    public abstract class NbscwMPATestBase : AbpIntegratedTestBase<NbscwMPATestModule>
    {
        ...
        protected void UsingDbContext(Action<NbscwMPADbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<NbscwMPADbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<NbscwMPADbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<NbscwMPADbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<NbscwMPADbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<NbscwMPADbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<NbscwMPADbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<NbscwMPADbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<NbscwMPADbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<NbscwMPADbContext, Task<T>> func)
        {
            T result;
